...
**man guckt einfach, welche Spaltennamen nicht in den anderen Datensätzen vorkommen**  
**to do**: (zu) schwer?! - eigentlich nicht - das Prinzip wurde ja schon gezeigt:

  - Unfalldaten von <https://unfallatlas.statistikportal.de/> abgerufen am 13.09.2024  
  Variable des Straßenzustands: Die heißt mal "STRZUSTAND", mal "USTRZUSTAND" und mal "IstStrassenzustand". Alle Werte eines Jahres in einer Spalte speichern und Durchschnitt berechnen.

**zum Aufklapper Musterlösung machen**
...
# Die unstimmigen Spaltennamen identifizieren
...
for i in range(0, df_of_columns.shape[1]):
  ...
  for vergleichsspalte in vergleichs_df: # vergleichsspalte ist der Spaltenname (als int) 2020, 2021 usw.
    ...
    if maske_fehlender_spaltennamen.sum() > 0:
      ...
      print(f"In den Unfalldaten {df_of_columns.columns[i]} treten die Spaltennamen {aktuelle_spalte[maske_fehlender_spaltennamen].values} auf, die nicht im Objekt {vergleichs_df.columns[j]} enthalten sind.\n")

    j +=1

```

**Output für die Ausgabe verschlanken, die auftretenden Spaltennamen in einer Series speichern und unique ausgeben.**
:::



**-> die Baugenehmigungen können zu den Übungen.**

  - Baugenehmigungen Statistisches Bundesamt: https://www-genesis.destatis.de/datenbank/online/table/31111-0006/sequenz=tabelleErgebnis&selectionname=31111-0006&zeitscheiben=1
...
print(f"NumPy-Array:/t\t{np_array_with_none}") 
...
Das Statistische Amt der Europäischen Union Eurostat führt die amtlichen europäischen Statistiken. In der [bei Eurostatt verfügbaren](https://ec.europa.eu/eurostat/statistics-explained/index.php?title=Natural_gas_price_statistics#Natural_gas_prices_for_non-household_consumers) Datei [Direktlink auf XLSX-Datei](https://ec.europa.eu/eurostat/statistics-explained/images/3/32/SE_figures_for_Gas_2023S2_v3.xlsx) sind die Preise für Erdgas für die Mitgliedstaaten und verschiedene mit der Europäischen Union assoziierte Staaten erfasst. Lesen Sie das Tabellenblatt 'Table 1' entweder so ein, dass 
  ...
  - in entweder in einer zweidimensionalen Struktur jede Zeile eine Beobachtung und jede Beobachtung eine Zeile sowie jede Spalte eine Variable und jede Variable eine Spalte ist.  
...
| Daten | Dateiname |
|---|---|
| Preise für Erdgas | SE_figures_for_Gas_2023S2_v3.xlsx |
:::
