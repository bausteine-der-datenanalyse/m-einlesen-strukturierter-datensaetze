...
# Strukturierte Datensätze einlesen: die Module NumPy und Pandas {#sec-numpypandas}
...
**die grundlegenden Informationen werden in den w-Python ausgelagert: Module, Epoche, [grundlegende Zeittypen](https://pandas.pydata.org/docs/user_guide/timeseries.html#overview) (pd.Timestamp usw.)**

{{< include _zeitreihen-python.md >}}

### Datumsformate in NumPy einlesen

**to do: eventuell überflüssig, weil der selbe Datentyp verwendet wird.**

### Datumsformate in Pandas einlesen
**Hier geht es weiter**

**Der Hinweis ist erst mal nur für mich. Das kann doch nicht so kompliziert sein?! Die Dokumentation des Moduls ist auch interessant. Es löst einige Probleme mit dem Ende der Sommerzeitumstellung. <https://pythonhosted.org/pytz/>**

:::: {#wrn-datetimetimezones .callout-warning appearance="simple" collapse="false"}
## andere Zeitzonen
...
**pd.Timestamp überflüssig, weil pd.to_datetime() (fast) das gleiche kann? pd.to_datetime kann auch string-Parsing, das kann pd.Timestamp nicht. Allerdings sollte man man pd.Timestamp kennen, weil das der Rückgabewert für Skalare ist.**
In Pandas können Zeitpunkte und Zeitdifferenzen:

  - Zeitpunkte (Timestamp): `pd.to_datetime()` oder `pd.date_range()`, Datentyp `datetime64[ns]` oder `datetime64[ns, tz]`

  - Zeitdifferenzen (Timedelta): `pd_to_timedelta()` oder `pd.timedelta_range`, Datentyp `timedelta64[ns]`


`pd.to_datetime()` erzeugt Werte des Datentyps `datetime64[ns]` und akzeptiert als Eingabewerte:

  - datetime-Objekte anderer Module.

    - time: für die Umwandlung eines time.struct_time Objekts muss dieses zuvor mit der Methode `time.strftime()` in einen string umgewandelt werden: `pd.to_datetime(time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(0)))` 

    - datetime: pd.to_datetime(datetime.datetime.fromtimestamp(0))

    - NumPy: `pd.to_datetime(np.datetime64(0, 's'))`

  - Dictionary bzw. DataFrame.

    - pd.to_datetime({'year':[2020, 2024], 'month': [1, 11], 'day': [1, 21]})

    - pd.to_datetime(pd.DataFrame({'year':[2020, 2024], 'month': [1, 11], 'day': [1, 21]}))

  - Zahlen (relativ zur Epoche, Standard sind Nanosekunden) `pd.to_datetime(1, unit = None)`. Das Argument `unit` nimmt die Werte 'ns', 'ms', 's', 'm', 'h', 'D', 'M', 'Y' für Nanosekunde, Millisekunde, Sekunde, Minute, Stunde, Tag, Monat bzw. Jahr entgegen.

  - Zeichenketten, die ein Datum oder ein Datum mit Uhrzeit ausdrücken.

      - In Standardformatierung: pd.to_datetime('2017-01-01'), `pd.to_datetime('2017-01-01T00')` oder `pd.to_datetime('2017-01-01 00')`, `pd.to_datetime('2017-01-01 00:00')` und `pd.to_datetime('2017-01-01 00:00:00')` sind äquivalent.

      - In beliebigen Formatierungen mit dem Argument `format = "%d/%m/%Y"`. See strftime documentation <https://docs.python.org/3/library/datetime.html#strftime-and-strptime-behavior>


**Ergänzen: Übung mit string-Formating aus dem a-Energiedatenanalyse.**
#### String-Formating
Argument format =  
...
#### Zugriff auf datetime-Objekte
 
Eine Übersicht aller verfügbaren Attribute und Methoden liefert  `dir(pd.to_datetime(0))`
``` {python}
# Attribute
...
# Methoden
print("Datum:", pd.to_datetime(0).date())
...
``` {python}
## Zugriff mit .dt --> klappt nur für pd.Series
print("Wochentag (0-6):", pd.Series(pd.to_datetime(0)).dt.weekday)
```


Mit der Methode (?) `.dt ` können die Informationen als Attribut abgrufen werden, bswp. `datetimeSerie.dt.weekday`

Noch einmal prüfen: Basiert nur NumPy oder alle auf der Internationalen Atomzeit (TAI für französisch Temps Atomique International), die keine Schaltsekunden kennt.  Die TAI ist die Grundlage für die Koordinierte Weltzeit UTC, die sich seit 1972 um eine ganzzahlige Zahl von Sekunden von TAI unterscheidet. Derzeit (Stand Januar 2020) gilt UTC = TAI − 37 Sekunden. https://de.wikipedia.org/wiki/Internationale_Atomzeit <https://numpy.org/doc/stable/reference/arrays.datetime.html>

**Aufgabe Zugriff auf datetime-Objekte** Schauen Sie in die Dokumentation von [Pandas](https://pandas.pydata.org/docs/user_guide/timeseries.html#time-date-components).

  - Welcher Wochentag war am "Anbeginn der Zeit"

  - War der "Anbeginn der Zeit" ein Schaltjahr?


### Von Magie und Drachen: Zeitzonen und Kalender
Zeitzonen, Sommerzeit und Schaltjahren 

Die Epoche des time-Moduls ist die Unix-Zeit und beginnt am 1. Januar 1970, 00:00:00 Uhr UTC. UTC steht für Coordinated Universal Time (auch bekannt als Greenwich Mean Time). Das Kürzel UTC ist ein Kompromiss für die englische und die französische Sprache. 
...
In NumPy: "Datetimes are always stored with an epoch of 1970-01-01T00:00. This means the supported dates are always a symmetric interval around the epoch, called “time span” in the table below."


#### NumPy
Noch einmal prüfen: Basiert [Nur NumPy oder alle?] auf der Internationalen Atomzeit (TAI für französisch Temps Atomique International), die keine Schaltsekunden kennt.  Die TAI ist die Grundlage für die Koordinierte Weltzeit UTC, die sich seit 1972 um eine ganzzahlige Zahl von Sekunden von TAI unterscheidet. Derzeit (Stand Januar 2020) gilt UTC = TAI − 37 Sekunden. https://de.wikipedia.org/wiki/Internationale_Atomzeit <https://numpy.org/doc/stable/reference/arrays.datetime.html>

  - date units are years (‘Y’), months (‘M’), weeks (‘W’), and days (‘D’)

  - time units are hours (‘h’), minutes (‘m’), seconds (‘s’), milliseconds (‘ms’), and some additional SI-prefix seconds-based units. [sollte bis Attosekunde gehen]
...
Ausrechnen, wie alt man ist: np.datetime64('2024-10-04') - np.datetime64('1984-07-26')
numpy.timedelta64(14680,'D')

Find the difference

some date - XY = some other date
