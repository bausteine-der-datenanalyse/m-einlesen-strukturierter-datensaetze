...
Erzeugt ein Array vom Typ `DatetimeIndex` mit dtype `datetime64`. Genau drei der folgenden vier Argumente sind für die Erzeugung erforderlich: 
...
print("\nWie viel Tage hat die Woche?", np.timedelta64(1,'W') / np.timedelta64(1,'D'))
```
...
**Wie alt sind Sie in Tagen? Wie alt in Sekunden? Rechnen Sie mit NumPy oder Pandas. Ergänzen: Musterlösung in Pandas**
...
**eventuell wieder panel-tabset für NumPy und Pandas**

#### NumPy
In NumPy muss man die Objekte aufteilen bzw. erst einmal alles als string einlesen und dann die Überschriften abtrennen und den Datensatz nach Datentypen in einzelne Objekte auftrennen (das haben wir schon in der Vorlesung zu NumPy gemacht).



#### Pandas
Zunächst wird die Datei der Großhandelspreise mit der Funktion `pd.read_csv()` eingelesen. Durch vorherige Betrachtung mit einem Tabellenkalkulationsprogramm konnte das Semikolon als Zeichentrenner identfiziert werden, das mit dem Argument `sep = ';'` übergeben wird. Durch Aufruf der Funktionen `pd.info()` und `pd.head()` wird der Erfolg kontrolliert.
...
Die Spalte DE/AT/LU [€/MWh] Originalauflösungen wird nicht als float erkannt. In der Ausgabe ist zu sehen, dass wenigstens in den ersten Zeilen fehlende Werte durch '-' markiert sind. Mittels der Methode `.describe()` kann überprüft werden, ob die Spalte überhaupt numerische Werte enthält.
...
Wenn der innere Aufbau einer Datei bekannt ist, können die notwendigen Parameter auch direkt beim Einlesen übergeben werden (Argumente `usecols`, `parse_dates` und `date_format`).
...
print(f"\nDurchschnittspreis werktags: {preis_werktags:.2f} [€/MWh]\tDurchschnittspreis am Wochenende: {preis_wochenende:.2f} [€/MWh]")
```

:::

#### pd.Series.resample
http://pandas.pydata.org/pandas-docs/dev/user_guide/timeseries.html#resampling

#### Übungen
Welcher Wochentag war am ersten und letzten Tag der Zeitreihe?

Daten aggregieren: Was war der Durchschnittspreis an Werktagen (Montag - Freitag), was am Wochenende? Was der über das gesamte Jahr der durchschnittliche Preis je Stunde?

--> als datetime64 einlesen, Maske mit .weekday-Abfrage und Mittelwert bilden. 
 <http://pandas.pydata.org/pandas-docs/dev/user_guide/timeseries.html#resampling> resample() is a time-based groupby, followed by a reduction method on each of its groups. 
