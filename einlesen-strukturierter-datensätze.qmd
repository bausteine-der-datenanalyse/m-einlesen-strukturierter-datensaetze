...
Der Bildhintergrund hat in allen Kanälen, auch im Alphakanal, den Farbwert 0. Dieser Teil des Bildes ist deshalb vollständig transparent und wird vom Hintergrund der Internetseite ausgefüllt. Dies ist aber bei einem weißen Hintergrund nicht zu erkennen.
...
Für beide Module haben sich Kürzel etabliert:
...
print("Struktur:", daten_empty_lines2.shape)
...
Weitere Werte können mit dem Argument `na_values = []` als fehlende Werte definiert werden. Mit dem Argument `keep_default_na = False` kann festgelegt werden, dass ausschließlich die in `na_values = []` übergebenen Werte als fehlende Werte interpretiert werden sollen. Standardmäßig werden mit dem Argument `na_filter = True` auch leere Zellen als NA eingelesen. ([Pandas Dokumentation](https://pandas.pydata.org/docs/user_guide/io.html#io-navaluesconst))  
**testen**  
**Es gibt auch das Argument `skip_blank_lines=True`** 
...
Pandas verwendet abhängig vom Datentyp verschiedene Werte zur Kennzeichnung fehlender Werte.
  ...
  - `NA` für Zeichenketten und Ganzzahlen. Der Datentyp bleibt erhalten. `NA` kann zwar auch als fehlender Wert für Gleitkommazahlen und andere NumPy Datentypen verwendet werden. Allerdings wird dafür ein erweiterter Pandas Dateityp benötigt (siehe das folgende Beispiel).
...
for element in bool_values:
  try:
    ...
    print(element, bool_value)

```

Dies gilt auch für den Identitätsabgleich.
``` {python}
...
Das Modul Pandas bietet wie das Modul NumPy verschiedene Funktionen, um mit fehlenden Werten zu arbeiten.

  - erzeugen: `pd.NA` - hier kommt es auf die Groß- und Kleinschreibung an. pd.na funktioniert nicht
  
  - fehlende Werte prüfen: pd.isnull(), isna(), notna(). isna() und notna() erkennen auch `None` als fehlenden Wert.

  - fehlende Werte finden: ?!

  - ersetzen: pd.fillna(), pd.ffill(), pd.bfill() 
...
**Aufgabe von Jean-Luca: Bestimmen Sie die Kodierung fehlender Werte**

  - mit Pandas geht das ganz leicht mit (.describe)

  - wie ginge es mit NumPy?! Oder sollte das Ziel sein, dass beide Pakete sich gut ergänzen?  
  "Numpy arrays are specifically used when complex scientific computation has to be performed whereas DataFrames are used mostly in data pre-processing." Das, Adrita 2022: Difference Between Pandas Dataframe and Numpy Arrays <https://www.askpython.com/python/pandas-dataframe-vs-numpy-arrays>

Datensatz vom Deutschen Wetterdienst Solarstrahlung (global/diffus) und der atmosphärischen Gegenstrahlung für Deutschland (Kodierung fehlender Werte mit -999) `produkt_st_stunde_20230831_20240630_01303.txt` 
