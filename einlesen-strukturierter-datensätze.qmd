...
::: {#nte-masked-arrays .callout-note}
## Warnhinweis maskierte Arrays

In der Dokumentation steht eine Warnung, dass man sich nicht darauf verlassen kann, dass maskierte Werte nicht von Operationen manipuliert werden. Welche Operationen dies betrifft, ist nicht angegeben.
...
**Lukas fragen, was sinnvoll zu beüben ist.**
...
mein_array = np.arange(1, 100 + 1).reshape(10, 10)
mein_array = ma.masked_where(condition = (mein_array % 2 != 0) & (mein_array % 3 == 0), a = mein_array)
print(mein_array, "\n")
```

``` {python}
# 2. Summen bilden
print(f"Summe der maskierten Werte: {mein_array.data[mein_array.mask].sum()}\tSumme der unmaskierten Werte: {mein_array.sum()}\n")
```

``` {python}
# 3. Maske zurücksetzen
mein_array.mask = False
print(mein_array, "\n")
```
...
(siehe auch bei den masked array Operationen ... soften ... harden)
