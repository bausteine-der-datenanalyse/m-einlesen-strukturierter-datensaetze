...
**hierhier Daten nachweisen**

Electricity Prices: table_531.xlsx Tabellenblatt "5.3.1 (excl. taxes)" einlesen
Department for Energy Security & Net Zero 2024: Energy Prices International Comparisons.
Industrial electricity prices in the IEA. https://www.gov.uk/government/uploads/system/uploads/attachment_data/file/670121/table_531.xls 30.09.2024 

  - falls die Übung überhaupt relevant ist - sollten alles Floats sein, außer die letzten beiden Spalten Integer

Überleitung: Eine als string eingelesene Spalte weist häufig auf fehlende Werte hin, die durch Sonderzeichen gekennzeichnet sind.

**ergänzen: np.nonzero() gibt ein tuple zurück, dass man zu einer Liste auflösen muss, um auf die Werte zuzugreifen: `indexposition = np.nonzero(taxes['Republic of Türkiye'] == '..')[0].tolist()`**
...
``` {python}
# Werte in Spalte 'Republic of Türkiye' ansehen
...
## chained gibt es die Fehlermeldung
## deprecated: df["col"][row_indexer] = value
## Use `df.loc[row_indexer, "col"] = values` instead
# taxes['Republic of Türkiye'].iloc[ indexposition ] = np.NaN

## unchained ist es schwierig
indexposition = np.nonzero(taxes['Republic of Türkiye'] == '..')[0].tolist()
print(indexposition)
...
Das Modul NumPy bietet Funktionen, um mit fehlenden Werten zu arbeiten.
  ...
  - `np.nonzero(np.isnan(array))` gibt ein Array mit den Indexpositionen der Elemente mit dem Wert 'NaN' zurück.
...
produkt_st_stunde_20230831_20240630_01303.txt

  - mit Pandas geht das ganz leicht mit (.describe)

  - wie ginge es mit NumPy?! **Tipp ist, dass sich beide Pakete ergänzen.  **
