...
Die Datensätze folgen dem Namensschema HRFC_COM_FR
  ...
  - FR = Flash Rates [RF = Raw Flashes, 1SF = Scaled Flashes]
...
Ein Dataset-Objekt wird mit der Funktion `nc.Dataset(filename, mode)` geöffnet. Mit dem Argument `filename` wird der Dateipfad übergeben. Im Argument wird der Zugriffsmodus festgelegt.
...
Die wesentlichen Informationen können mit der print-Funktion ausgegeben werden `print(cdf)`. Die Ausgabe ist besonders für komplexere Dateien jedoch unübersichtlich.
...
Die Variablen (die Datensätze) können mit dem Attribut `print(cdf.variables)` ausgegeben werden, die Ausgabe ist ebenfalls ein Dictionary.
